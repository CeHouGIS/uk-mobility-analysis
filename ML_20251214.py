...
BASE_FEATURE_COLS = ['days_t1', 'days_t3', 'days_t5', 'days_t6', 'days_t8', 'days_t9']
...
def load_and_filter_data(path: str, select_home_same: bool) -> pd.DataFrame:
    ...
    df_select = df[(df['home_hours_2020'] > 7) & (df['home_hours_2020'] < 22)]
    df_select = df_select[(df_select['workplace_days_2020'] > 2) & (df_select['workplace_days_2020'] < 6)]
    activity_cols_2020 = [f'{col}_2020' for col in BASE_FEATURE_COLS]
    df_select = df_select[~(df_select[activity_cols_2020].sum(axis=1) == 0)].reset_index(drop=True)
    
    if select_home_same == True:
        df_select = df_select[df_select['home_same'] == 0]
    
    print(f"      Data shape after filtering: {df_select.shape}")
    return df_select
